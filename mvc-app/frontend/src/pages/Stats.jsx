...
import { getShowsByBand, getBandFollowerCount, getShowInterestedCount } from '../api'
...
export default function Stats() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // shows always load — other counts are best-effort
        const showsData = await getShowsByBand(bandId)
        setShows(showsData)

        // follower count — don't let a failure block the page
        getBandFollowerCount(bandId)
          .then(setFollowerCount)
          .catch(() => setFollowerCount(0))

        // interested count per show — individual failures default to 0
        const entries = await Promise.all(
          showsData.map(async (s) => {
            const n = await getShowInterestedCount(s.showId).catch(() => 0)
            return [s.showId, n]
          })
        )
        setInterestedCounts(Object.fromEntries(entries))
      } catch {
        // shows failed — page still renders, just empty
      } finally {
        setLoading(false)
      }
    }
  ...
  const totalInterested = Object.values(interestedCounts).reduce((a, b) => a + b, 0)
  ...
  const showsSorted = [...shows].sort((a, b) => new Date(b.date) - new Date(a.date))
      ...
      <div className="relative z-20">
        ...
        <div className="max-w-4xl mx-auto px-4 pt-28 pb-28 space-y-6">
          ...
          <div className="backdrop-blur-xl bg-white/70 dark:bg-white/10 border border-black/[0.08] dark:border-white/20 rounded-3xl p-8 relative overflow-hidden shadow-[0_0_60px_rgba(168,85,247,0.12)]">
            ...
            <p className="text-gray-400 dark:text-white/40 text-sm mt-2">Live data from your shows and followers</p>
          </div>

          {loading ? (
            <div className="text-gray-400 dark:text-white/30 text-sm tracking-widest uppercase text-center py-16">Loading…</div>
          ) : (
            <>
              {/* Audience row — followers + interested */}
              <div className="grid grid-cols-2 gap-4">
                <div className="backdrop-blur-xl bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-400/40 rounded-2xl p-6 shadow-[0_0_50px_rgba(168,85,247,0.18)]">
                  <div className="text-purple-500 dark:text-purple-400 text-xs tracking-widest uppercase mb-2">Followers</div>
                  <div className="text-gray-900 dark:text-white font-black text-4xl">{followerCount ?? '—'}</div>
                  <div className="text-gray-400 dark:text-white/40 text-xs mt-1">customers following you</div>
                </div>
                <div className="backdrop-blur-xl bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-400/40 rounded-2xl p-6 shadow-[0_0_50px_rgba(168,85,247,0.18)]">
                  <div className="text-purple-500 dark:text-purple-400 text-xs tracking-widest uppercase mb-2">Total Saves</div>
                  <div className="text-gray-900 dark:text-white font-black text-4xl">{totalInterested}</div>
                  <div className="text-gray-400 dark:text-white/40 text-xs mt-1">across {shows.length} show{shows.length !== 1 ? 's' : ''}</div>
                </div>
              </div>
                ...
                {nextShow ? (
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {interestedCounts[nextShow.showId] != null && (
                        <p className="text-gray-400 dark:text-white/40 text-xs mt-2">
                          ♥ {interestedCounts[nextShow.showId]} interested
                        </p>
                      )}
                    </div>
              ...
              {/* All shows list with interested counts */}
              {shows.length > 0 && (
                <div className="backdrop-blur-xl bg-white/70 dark:bg-white/10 border border-black/[0.08] dark:border-white/20 rounded-3xl p-6 shadow-[0_0_40px_rgba(168,85,247,0.08)]">
                  <h3 className="text-gray-900 dark:text-white font-bold text-sm tracking-widest uppercase mb-4 pb-3 border-b border-black/[0.06] dark:border-white/10">
                    All Shows
                  ...
                  <div className="space-y-2">
                    {showsSorted.map(show => {
                      ...
                      const saved = interestedCounts[show.showId] ?? 0
                      return (
                        <div key={show.showId} className="flex items-center justify-between gap-4 py-3 border-b border-black/[0.04] dark:border-white/5 last:border-0">
                          <div className="min-w-0">
                            <div className="text-gray-900 dark:text-white text-sm font-semibold truncate">{show.location ?? '—'}</div>
                            <div className="text-gray-400 dark:text-white/40 text-xs">{formatDate(show.date)}</div>
                          </div>
                          <div className="flex items-center gap-3 flex-shrink-0">
                            {show.genre && (
                              <span className="text-purple-500 dark:text-purple-300 text-xs tracking-widest uppercase hidden sm:inline">{show.genre}</span>
                            )}
                            <span className="text-gray-400 dark:text-white/40 text-xs flex items-center gap-1">
                              ♥ <span className="text-gray-900 dark:text-white font-bold">{saved}</span>
                            </span>
                            <span className={`text-xs tracking-widest uppercase px-2 py-0.5 rounded-full border ${
                              isUpcoming
                                ...
                                : 'bg-black/5 dark:bg-white/5 border-black/[0.08] dark:border-white/10 text-gray-400 dark:text-white/30'
                            }`}>
