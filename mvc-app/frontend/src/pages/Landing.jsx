...
<div className='backdrop-blur bg-white/10 border border-white/20 rounded-3xl fixed bottom-18 left-4 right-4 sm:left-10 sm:right-auto sm:w-108 z-30 p-6 shadow-[0_0_40px_rgba(168,85,247,0.15)] overflow-hidden flex flex-col gap-4'>
  ...
  <div className='flex items-center justify-between gap-4'>
    ...
    <div className='flex flex-col justify-center gap-2'>
      {isLoggedIn ? (
        <>
          <button onClick={() => navigate('/dashboard')} className='bg-purple-600/60 hover:bg-purple-600 backdrop-blur-sm border border-purple-400/40 text-white text-xs tracking-widest uppercase px-6 py-2 rounded-full transition-all whitespace-nowrap'>
            Dashboard
          </button>
          <button onClick={handleLogout} className='bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white text-xs tracking-widest uppercase px-6 py-2 rounded-full transition-all whitespace-nowrap'>
            Log Out
          </button>
        </>
      ) : (
